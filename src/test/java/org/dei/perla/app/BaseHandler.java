...
public class BaseHandler extends Observable implements StatementHandler {
	...
	public synchronized void data(Statement s, Object r) {
		...
		if (s instanceof SelectionStatement) {
			...
			for (Object o : (Object[]) r) {
				...
				d = o.getClass();
				d.cast(o);
				System.out.println("Get data queryn: " + this.id + " samplen :" + this.count + " value :" + o + "type :"
	...
	public void data(Statement s, Record r) {
		...
		/*
		 * QUANDO ARRIVANO I DATI COSTRUISCO LA QUERY E LI INSERISCO NEL
		 * DATABASE Per controllare l'inserimento vai sulla riga di comando di
		 * mysql scrivi SELECT * FROM nometabella che ti compare nella console
		 * di eclipse
		 * 
		 * 
		 * if (db==null){ db=new DatabaseClass(); try { db.connect(); } catch
		 * (SQLException e) { // TODO Auto-generated catch block
		 * e.printStackTrace(); } } String insertion = "INSERT INTO "+tableName
		 * +"(";
		 * 
		 * int numValues=r.getValues().length;
		 * 
		 * for (int i=0; i<numValues; i++){ if (i!=numValues-1){
		 * insertion=r.getFields().get(i).getId()+","; }else{
		 * insertion=insertion+r.getFields().get(i).getId(); } }
		 * insertion=insertion+") VALUES (";
		 * 
		 * for (int i=0; i<numValues; i++){ if (i!=numValues-1){
		 * insertion=insertion+r.getValues()[i]+","; }else{
		 * insertion=insertion+r.getValues()[i]; } }
		 * 
		 * insertion=insertion+");"; System.out.println(insertion); try {
		 * db.saveData(insertion); } catch (SQLException e) { // TODO
